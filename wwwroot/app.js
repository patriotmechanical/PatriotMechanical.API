...
async function loadDashboard() {
    ...
    document.getElementById("kpiNoInvoiceSub").innerText = noInvoiceCount === 1 ? "open job" : "open jobs";
...
function renderOpsStats(data) {
    ...
    window._opsStats = [
        ...
        { label: "Open, No Invoice", count: data.completedNoInvoiceCount ?? (data.completedNoInvoice ? data.completedNoInvoice.length : 0), color: "#ea580c", items: data.completedNoInvoice || [], type: "noinvoice" },
    ];
}
...
function openOpsDrilldown(idx) {
    ...
    if (stat.type === "noinvoice") html += "<th>Status</th><th class='text-right'>Job Total</th>";
    html += "</tr></thead><tbody>";
    ...
    stat.items.forEach(item => {
        ...
        if (stat.type === "wo") {
            ...
            html += `<tr${custClick}><td class="bold">${item.jobNumber || "—"}</td><td>${item.customerName || "—"}</td><td><span class="status-badge open">${item.status || "—"}</span></td><td class="text-right danger">${total}</td></tr>`;
        }
    });
