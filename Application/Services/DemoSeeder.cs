...
namespace PatriotMechanical.API.Application.Services
{
    public static class DemoSeeder
    {
        ...
        public static async Task ResetDemoDataAsync(AppDbContext db)
        {
            ...
            // Use local variables for LINQ (EF Core .NET 10 can't evaluate static fields in expressions)
            var companyId = DemoCompanyId;
            var userId = DemoUserId;
            var email = DemoEmail;

            // Ensure demo company
            var hasCompany = await db.CompanySettings.AnyAsync(c => c.Id == companyId);
            if (!hasCompany)
            {
                await db.Database.ExecuteSqlRawAsync(@"
            ...
            // Ensure demo user
            var hasUser = await db.Users.AnyAsync(u => u.Id == userId);
            if (!hasUser)
            {
