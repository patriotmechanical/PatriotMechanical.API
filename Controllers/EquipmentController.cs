    ...
    public class EquipmentController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetAll()
        {
            ...
            var equipment = await _context.Equipment
                ...
                .Select(e => new
                {
                    e.Id, e.Type, e.Brand, e.ModelNumber, e.SerialNumber,
                    e.InstallDate, e.WarrantyExpiration, e.WarrantyRegistered, e.Notes,
                    CustomerName = e.Customer.Name, CustomerId = e.CustomerId,
        ...
        {
            if (equipment.CustomerId == Guid.Empty) return BadRequest("Customer is required.");
            if (string.IsNullOrWhiteSpace(equipment.Type)) return BadRequest("Equipment type is required.");
            ...
            if (equipment.InstallDate.HasValue) equipment.InstallDate = DateTime.SpecifyKind(equipment.InstallDate.Value, DateTimeKind.Utc);
            if (equipment.WarrantyExpiration.HasValue) equipment.WarrantyExpiration = DateTime.SpecifyKind(equipment.WarrantyExpiration.Value, DateTimeKind.Utc);
