...
public class ApController : ControllerBase
{
    ...
    public async Task<IActionResult> AddBill([FromBody] ApBill bill)
    {
        ...
        if (bill.VendorId == Guid.Empty) return BadRequest("Vendor is required.");
        bill.Id = Guid.NewGuid();
        bill.DueDate = DateTime.SpecifyKind(bill.DueDate, DateTimeKind.Utc);
        bill.CreatedOn = DateTime.UtcNow;
        bill.IsPaid = false;
        if (bill.TotalAmount <= 0) bill.TotalAmount = bill.Amount;
    ...
    {
        var vendor = await _context.Vendors.Include(v => v.Bills).FirstOrDefaultAsync(v => v.Id == id);
        if (vendor == null) return NotFound();
        var bills = vendor.Bills.Where(b => !b.IsPaid).OrderBy(b => b.DueDate)
            .Select(b => new { b.Id, b.Amount, b.TotalAmount, b.DueDate, b.IsPaid });
        return Ok(new { vendor.Id, vendor.Name, Bills = bills });
    }
    ...
    {
        if (string.IsNullOrWhiteSpace(vendor.Name)) return BadRequest("Vendor name is required.");
    ...
    public async Task<IActionResult> GetApSummary()
    {
        ...
        var summary = await _context.Vendors
            ...
            {
                v.Id, v.Name,
                TotalInvoiceAmount = v.Bills.Where(b => !b.IsPaid).Sum(b => (decimal?)b.TotalAmount) ?? 0,
                AmountDueNow = v.Bills.Where(b => !b.IsPaid).Sum(b => (decimal?)b.Amount) ?? 0,
                NextDue = v.Bills.Where(b => !b.IsPaid).OrderBy(b => b.DueDate).Select(b => b.DueDate).FirstOrDefault()
            })
    ...
    {
        var badBills = await _context.ApBills.Where(b => b.TotalAmount <= 0).ToListAsync();
        foreach (var bill in badBills) bill.TotalAmount = bill.Amount;
        await _context.SaveChangesAsync();
