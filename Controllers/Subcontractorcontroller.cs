...
namespace PatriotMechanical.API.Controllers
{
    ...
    public class SubcontractorController : ControllerBase
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(sub.Name)) return BadRequest("Name is required.");
            sub.Id = Guid.NewGuid();
            sub.IsActive = true;
        ...
        public async Task<IActionResult> GetEntriesForSub(Guid id)
        {
            ...
            var entries = await _context.SubcontractorEntries
                ...
                .Include(e => e.WorkOrder).ThenInclude(w => w.Customer)
                .OrderByDescending(e => e.Date)
                .Select(e => new
                {
                    e.Id, e.Date, e.Hours, e.HourlyRate,
                    TotalCost = e.Hours * e.HourlyRate, e.Notes,
            ...
            return Ok(new
            {
                sub.Id, sub.Name, sub.Company, sub.Trade,
        ...
        {
            var workOrder = await _context.WorkOrders.Include(w => w.Customer).FirstOrDefaultAsync(w => w.Id == workOrderId);
            ...
            var entries = await _context.SubcontractorEntries
                ...
                .Select(e => new
                {
                    e.Id, e.Date, e.Hours, e.HourlyRate,
                    TotalCost = e.Hours * e.HourlyRate, e.Notes,
                    SubName = e.Subcontractor.Name, SubCompany = e.Subcontractor.Company,
            ...
            return Ok(new
            {
                workOrder.Id, workOrder.JobNumber,
        ...
        public async Task<IActionResult> LogHours([FromBody] SubcontractorEntry entry)
        {
            ...
            await _context.SaveChangesAsync();
            return Ok(new { entry.Id, entry.Hours, entry.HourlyRate, TotalCost = entry.Hours * entry.HourlyRate, entry.Date, entry.Notes });
        }
